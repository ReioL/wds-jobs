...
export async function insertJobListing(jobListing: typeof JobListingTable.$inferInsert) {
  const [newListing] = await db
    ...
    .values(jobListing)
    .returning({ id: JobListingTable.id, organizationId: JobListingTable.organizationId });
  revalidateJobListingCache(newListing);

  return newListing;
}

export async function updateJobListingDb(id: string, jobListing: Partial<typeof JobListingTable.$inferInsert>) {
