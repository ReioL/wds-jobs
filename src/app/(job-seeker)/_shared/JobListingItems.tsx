...
async function DaysSincePosting({ postedAt }: { postedAt: Date }) {
  await connection(); // next knows that dynamic data is used, like date.now()
...
async function getJobListings(searchParams: z.infer<typeof searchParamsSchema>, jobListingId: string | undefined) {
  ...
  const data = await db.query.JobListingTable.findMany({
    where: or(
      jobListingId ? and(eq(JobListingTable.status, "published"), eq(JobListingTable.id, jobListingId)) : undefined,
